...
public enum PaymentSystem {
    ...
    private PaymentSystem(String cardLengthRange) {
        ...
        // idk yet whether it should throw an exception if an argument contains more
        // than one hyphen
    }
    ...
    private PaymentSystem(String cardLengthRange, String prefixRange) {
        ...
        if (prefixRange.indexOf("-") != -1) {
            ...
            IntStream.range(Integer.valueOf(rangeEnds[0]), Integer.valueOf(rangeEnds[1]) + 1).forEach(prefix -> {
    ...
    public List<List<Integer>> getPrefixesAsLists() {
        return prefixList.stream().map(n -> List.of(n.toString().split("")).stream().map(Integer::valueOf).toList())
                .toList();
    }
}
